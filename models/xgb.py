...
for n in tqdm([0.0001, 0.001, 0.01, 0.1, 0.2, 0.3]):
    # Initialize an XGBoost classifier with specified parameters
    xgb = XGBClassifier(n_estimators=20, max_depth=3, learning_rate=n)
    ...
    # Calculate the mean accuracy rate for the current learning rate
    acc_rate.append(statistics.mean(cv_acc))

# Plot the accuracy rates for different learning rates
plt.figure(figsize=(10, 6))
plt.plot([0.0001, 0.001, 0.01, 0.1, 0.2, 0.3], acc_rate, color='blue', linestyle='dashed', marker='o', markerfacecolor='red', markersize=10)
plt.title('Evolution of accuracy with respect to the learning rate')
