...
# Initialize HIVECOTEV2 classifier with specified parameters
hc2 = HIVECOTEV2(time_limit_in_minutes=2, n_jobs=-1, verbose=1)
...
for i, (X_test, y_test) in enumerate(tqdm(zip(df_list, labels_list), total=len(df_list))):
    ...
    # Fit the HIVECOTEV2 classifier on the training data
    net = hc2.fit(X_train, y_train)
